...
project.update_parameters(
    {
        "data": {
            ...
            "data_suffix": ".npy",
            "feature_suffix": "_videomae.npy" if features_only else "_with_pose.npy",
...
num_epochs = 200
ext = "" if features_only else "_with_pose"
for exp in list(search_results.keys()):
    ...
    for leave_ind in range(20):
        ...
        project.run_episode(
            f"exp_{exp}_videomae_{leave_ind}{ext}",
        ...
        try:
            project.run_episode(
                f"exp_{exp}_videomae_{leave_ind}{ext}_lr1e-5",
                load_episode=f"exp_{exp}_videomae_{leave_ind}{ext}",
                parameters_update=pars,
                force=True,
