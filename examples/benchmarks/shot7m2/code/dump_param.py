...
param = {
    "general": {
        ...
        "overlap": 60, # int; the overlap (in frames) between neighboring segments
        "interactive": False, # bool; if true, annotations are assigned and features are computed for pairs of clips (animals)
        "exclusive" : False,
    },
    "data": {
        "data_suffix": "_3D_poses.h5", # str; the data suffix (the data files should be named {video_id}{data_suffix}, e.g. video1_suffix.pickle, where video1 is the video is and _suffix.pickle is the suffix
        ...
        "annotation_suffix": "_annotations.pickle", # str | set, optional the suffix or the set of suffices such that the annotation files are named {video_id}{annotation_suffix}, e.g, video1_suffix.pickle where video1 is the video id and _suffix.pickle is the suffix
        "canvas_shape": [960,600], # list; [x, y] size of the canvas where the coordinates are defined
        ...
        "behaviors": ['activity_Episode_Casual', 'activity_Episode_Intense', 'activity_Episode_Dribbling', 'activity_Episode_Not_Playing', 'action_Idle', 'action_Move', 'action_Dribble', 'action_Hold',
                      'action_Feint', 'action_Shoot', 'action_LSpin', 'action_RSpin', 'action_Sprint', 'action_Force', 'action_LTurn', 'action_Rturn', 'moveme_Lfoot', 'moveme_Rfoot', 'moveme_Lhand', 'moveme_Rhand',
                      'moveme_Ball', 'moveme_Switch', 'moveme_Lelbow_flex', 'moveme_Lelbow_ext', 'moveme_Relbow_flex', 'moveme_Relbow_ext', 'moveme_Lknee_flex', 'moveme_Lknee_ext', 'moveme_Rknee_flex', 'moveme_Rknee_ext'], # list; the list of behaviors to put in the annotation (if null, if will be inferred from the data; !!PLEASE SET IT MANUALLY if different files can have different behavior sets!!)
        "filter_annotated": True, # bool; if true, the samples that do not have any labels will be filtered
        "filter_background": True, # bool, if true, only the unlabeled frames that are close to annotated frames will be labeled as background
    ...
    "training": {
        ...
        "val_frac": 0.75, # float; fraction of dataset to use as validation
    ...
    "features": {
        "keys": ["coords" ,"speed_joints", "acc_joints"], # set; a list of names of the features to extract (a subset of available keys; run project.help("features") for more info)
    ...
    "augmentations": {
        "augmentations": {'add_noise', 'zoom', 'shift', 'rotate'}, # set; a set of augmentations (from 'rotate', 'real_lens', 'add_noise', 'shift', 'zoom', 'mirror', 'switch')
